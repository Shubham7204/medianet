...
const AdvertiserChatBot: React.FC = () => {
  ...
  const formatBannerConcept = (data: any): { content: string; imageUrl?: string } => {
    ...
    const imageUrl =
      data.result?.image_url || data.image_url || data.result?.image_path || data.image_path;
      ...
      if (result.concept) {
        ...
        if (concept.cta || concept.CTA || concept["Call-to-Action"]) {
          formatted += `🔘 **Call-to-Action:** ${
            concept.cta || concept.CTA || concept["Call-to-Action"]
          }\n`;
        }

        if (concept.visual_design || concept["Visual Design Description"]) {
          formatted += `🎨 **Visual Design:** ${
            concept.visual_design || concept["Visual Design Description"]
          }\n`;
        }
        ...
        if (concept.creative_strategy || concept["Creative Strategy"]) {
          formatted += `💡 **Strategy:** ${
            concept.creative_strategy || concept["Creative Strategy"]
          }\n`;
        }

        if (concept.typography_suggestions || concept["Typography Suggestions"]) {
          formatted += `✏️ **Typography:** ${
            concept.typography_suggestions || concept["Typography Suggestions"]
          }\n`;
        }
      }
  ...
  const handleExaSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:8000/advertiser/exa-competitive-intelligence",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
              ...
              <CardContent className="p-0 h-full flex flex-col">
                ...
                <div className="border-t border-border/40 p-4 bg-card/30 backdrop-blur-sm">
                  <div className="flex gap-3 mb-3">
                    <div className="flex-1 relative">
                      ...
                      {(isWebSearchMode || isCampaignAnalysisMode || isBannerGenMode) && (
                        <div className="absolute -top-6 left-0 text-xs font-medium">
                          {isWebSearchMode && (
                            <span className="text-blue-600">🔍 Competitive Analysis Active</span>
                          )}
                          {isCampaignAnalysisMode && (
                            <span className="text-green-600">📊 Campaign Analysis Active</span>
                          )}
                          {isBannerGenMode && (
                            <span className="text-purple-600">🎨 Banner Generation Active</span>
                          )}
                        </div>
                      )}
                    </div>
                    ...
                    >
                      <BarChart
                        className={`h-4 w-4 ${isCampaignAnalysisMode ? "text-white" : ""}`}
                      />
                    </Button>
                    <Button
                      onClick={() => {
                    ...
                    <Badge
                      ...
                      onClick={() => {
                        ...
                        setBannerFormData({
                          ...
                          description:
                            "Diwali Sale campaign to promote Striver's premium courses. Target audience: students and professionals preparing for placements and coding interviews. Emphasize festive spirit with lights, diyas, and celebration.",
                          banner_size: "300x250",
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={e =>
                  setBannerFormData(prev => ({ ...prev, website_url: e.target.value }))
                }
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={e =>
                  setBannerFormData(prev => ({ ...prev, description: e.target.value }))
                }
            ...
            <div>
              <Label htmlFor="banner_size" className="text-foreground">
                Banner Size
              </Label>
              <Select
                value={bannerFormData.banner_size}
                onValueChange={value =>
                  setBannerFormData(prev => ({ ...prev, banner_size: value }))
                }
              >
            ...
            <div>
              <Label htmlFor="offer" className="text-foreground">
